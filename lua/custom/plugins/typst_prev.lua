    ...
    on_stderr = function(_, data)
      if data then
        for _, line in ipairs(data) do
          if line ~= '' and line:lower():match 'error' then vim.schedule(function() vim.notify('Typst: ' .. line, vim.log.levels.ERROR) end) end
        end
      end
    end,
    on_exit = function(_, code, _)
      typst_jobs[bufnr] = nil
      if code ~= 0 and code ~= 143 then vim.notify("Process 'typst watch' finished unexpectedly", vim.log.levels.ERROR) end
    end,
  })
...
vim.api.nvim_create_user_command('ToggleTypstPreview', toggle_typst_preview, { desc = "Toggle 'typst' watch and open compiled pdf in with SumatraPDF" })
